...
LastEditTime: 2022-01-28 20:02:15
...
def write_flo(filename, flow):
    ...
    print ("write size is : ", filename , " " ,flow.shape)
    magic.tofile(f)
    w.tofile(f)
    h.tofile(f)
...
def make_flow_for_one_gray_kitti_picture(args, path_1, path2):
    ...
    with torch.no_grad():
        ...
        print (flo_np.dtype)
        save_path = "/home/george/data_sets/00/flow/000000.flo"
        if flo_np.shape[0] != image_1.shape[0] or flo_np.shape[1] != image_1.shape[1] or flo_np.shape[2] != 2:
            print ("unpdding 失败")
        else:
            print ("unpadding 成功")
...
def make_flow_for_kitti_sets(args):
    ...
    with torch.no_grad():
        ...
        for imfile1, imfile2 in zip(images[:-1], images[1:]):
            ...
            flow_up = padder.unpad(flow_up)##恢复到padding之前的状态
            flo = flow_up[0].permute(1,2,0).cpu().numpy()
            print (flo.dtype)
            write_flo(save_path, flo)
            # flow_up = torch.from_numpy(flow_up)
...
if __name__ == '__main__':
    ...
    path1 = "/home/george/data_sets/00/image_0/000000.png"
    path2 = "/home/george/data_sets/00/image_0/000001.png"
